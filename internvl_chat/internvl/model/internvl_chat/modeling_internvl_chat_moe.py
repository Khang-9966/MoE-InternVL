...
from transformers import (AutoModel, GenerationConfig, LlamaForCausalLM,
                          LlamaTokenizer, Qwen2ForCausalLM, Qwen2MoeConfig, Qwen2MoeForCausalLM)
...
class InternVLChatModel(PreTrainedModel):
    ...
    _no_split_modules = ['InternVisionModel', 'LlamaDecoderLayer', 'InternLM2DecoderLayer',
                         'Phi3DecoderLayer', 'Qwen2DecoderLayer','Qwen2MoeDecoderLayer']
    ...
    def __init__(self, config: InternVLChatConfig, vision_model=None, language_model=None):
        ...
        if language_model is not None:
            ...
            elif config.llm_config.architectures[0] == 'Qwen2MoeModel':
                self.language_model = Qwen2MoeForCausalLM(config.llm_config)
            else:
    ...
    def wrap_llm_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        elif self.llm_arch_name in ['Qwen2MoeModel']:
            target_modules = ['self_attn.q_proj', 'self_attn.k_proj', 'self_attn.v_proj', 'self_attn.o_proj',
                              'mlp.shared_expert.gate_proj', 'mlp.shared_expert.down_proj', 'mlp.shared_expert.up_proj']+[
    ...
    "mlp.experts.3.down_proj"
        ...
        lora_config = LoraConfig(
            ...
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
            modules_to_save=["mlp.shared_expert_gate","mlp.gate"],
            task_type='CAUSAL_LM'
        )
    ...
    def forward(
        ...
        output_router_logits = (
            output_router_logits if output_router_logits is not None else self.config.llm_config.output_router_logits
        )
        ...
        if labels is not None:
            loss += self.language_model.router_aux_loss_coef * aux_loss.to(loss.device)  # make sure to reside in the same device
